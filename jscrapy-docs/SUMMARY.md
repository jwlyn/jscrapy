# Summary

* [jscrapy简介](docs/index.md)
* [特色功能](docs/important_feature.md)
* [jscrapy模块](docs/spider_component.md)

-----
* [快速开始](docs/quick_start/index.md)
    * [安装](docs/quick_start/install.md)
    * [配置](docs/quick_start/config.md)
	* [第一个爬虫程序](docs/quick_start/1st_task.md)

-----
* [用户手册](docs/user_manual/index.md)

-----
* [开发手册](docs/dev_guid/project_arch.md)
    * [架构](docs/dev_guid/project_arch.md)
    * [功能边界](docs/dev_guid/feature_arch.md)
	* [集群部署](docs/dev_guid/cluster_deploy_arch.md)
	* [单点部署](docs/dev_guid/single_deploy_arch.md)
	* [无损抓取队列结构](docs/dev_guid/rdbms_queue_table.md)

-----
* [杂项](docs/masstec/springboot-mybatis_use.md)

-----
* [关于](docs/about/about.md)

-----
* [使用的第三方库](docs/3party_library.md)

